...
export async function exportGraph(session, { operationTimeoutMs = OPERATION_TIMEOUT_DEFAULT_MS, budget = null } = {}) {
  const nodeResult = await session.run("MATCH (n) RETURN elementId(n) AS exportId, labels(n) AS labels, properties(n) AS properties", {}, neo4jTransactionConfig(remainingBudgetMs(budget, operationTimeoutMs, "graph node export")));
  const relResult = await session.run("MATCH (a)-[r]->(b) RETURN elementId(a) AS start, elementId(b) AS end, type(r) AS type, properties(r) AS properties", {}, neo4jTransactionConfig(remainingBudgetMs(budget, operationTimeoutMs, "graph relationship export")));
  return {
    codecVersion: GRAPH_CODEC_VERSION,
...
export async function graphCensus(session, { operationTimeoutMs = OPERATION_TIMEOUT_DEFAULT_MS, budget = null } = {}) {
  const labels = await session.run("MATCH (n) UNWIND labels(n) AS label RETURN label, count(*) AS count ORDER BY label", {}, neo4jTransactionConfig(remainingBudgetMs(budget, operationTimeoutMs, "graph label census")));
  const types = await session.run("MATCH ()-[r]->() RETURN type(r) AS type, count(*) AS count ORDER BY type", {}, neo4jTransactionConfig(remainingBudgetMs(budget, operationTimeoutMs, "graph relationship census")));
  const episodes = await session.run("MATCH (n:Episodic) RETURN CASE WHEN n.name STARTS WITH 'items:' THEN 'items' WHEN n.name STARTS WITH 'correction:' THEN 'correction' ELSE 'unsupported' END AS pattern, count(*) AS count ORDER BY pattern", {}, neo4jTransactionConfig(remainingBudgetMs(budget, operationTimeoutMs, "graph episode census")));
