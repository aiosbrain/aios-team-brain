...
| Undated document | Episode timestamp = the shared `resolveWorkTime` (`lib/ingest/work-time`: `committed_at` → `source_ts` → edit time → create time, spelling-insensitive) then `synced_at`, never `now()` (won't float to top of recency-ranked arcs). The projector previously read `source_ts` ALONE, so git commits — which carry `committed_at` — were graph-stamped at sync time and disagreed with the timeline |
| Tier reclassification (external→team) | Applies on an UNCHANGED re-push too (`content_sha256` covers only body+title): `ingestItem` heals `items.access` + cascades `tasks.audience`, so the Postgres read paths (retrieval, `/api/v1/items`, dashboard `visibleItems`/`visibleTasks`) stop serving the old tier immediately. The graph follows on the next projector tick — the healed `access` yields a new `group_id` → `tierChanged` → episodes deleted from the old group before re-push. That inline delete is best-effort, so the old group is also recorded in `pending_delete_group_id` (+ `pending_delete_at`, which anchors the 1h straggler grace so an ordinary content re-push can't restart the clock) and `reconcile` retries until it's verified empty (deep scan) — old tier can't stay searchable even if the inline delete fails (B2). Outstanding cleanups are reported as `pendingCleanups` in the `graph_project` `ingest_runs` meta; a count that never returns to 0 is a stuck cleanup, not bookkeeping. BOTH ingest paths (unchanged re-push AND body change) fan out through `lib/ingest/reclassify`, in a strict order: `cascadeInheritedAudience` (`tasks`, `extracted_facts`, `stakeholder_mentions`) runs BEFORE `items.access` is committed and `settleReclassification` after, so every failure is fail-closed and the next tick retries — committing the tier first would strand the inheriting rows at the old tier PERMANENTLY (the retry computes `accessChanged=false`). Caches: **narrowing purges** the external `arc_cache`/`work_timeline_cache` rows (both layers serve stale-while-revalidate, so a stale-mark would still hand the old payload to the next external viewer), **widening only stale-marks** them (incomplete, not leaking). For a NARROWING the external purge repeats from `lib/graph/run` once the episodes have actually left the external Graphiti group — arcs synthesize from that group, so a rebuild before then re-poisons the row and stamps it fresh; both signals (`externalGroupVacated`, `cleanedExternal`) are direction-aware so a widening never triggers a needless cold re-synthesis. The social chain follows too, but by a different rule: it inherits from EVIDENCE (jsonb `[{itemId,…}]`), so `narrowSocialChainForItem` re-applies the evidence CEILING — a narrowed item drops every `external` opportunity citing it, and its plans/variants/approvals/media/publications/analytics with it, each written by that table's own single-writer module. Anchored on the evidence match rather than the rows' current tier so the walk is idempotent — the statements autocommit, so a crash mid-walk must still be repairable by the retry. NARROWING ONLY: a ceiling is a limit, not a target, so widening an item never auto-publishes derived content |
| An `external`-tier key pushing at an existing `team`-tier item's path | Split on intent: an IDENTICAL body takes the normal unchanged path with the stored tier untouched (the benign race — a client connector re-pushing a doc narrowed upstream between syncs; erroring every tick would wedge its loop), a DIFFERENT body is REFUSED `403 forbidden_tier` (`TierViolationError`) — accepting it would overwrite internal content and carry the row's tier with it. Either way an existing item's tier is not writable by an untrusted pusher, and on the identical-body path its frontmatter and attribution are frozen too (the heal writes frontmatter wholesale, so a client key holding a since-narrowed doc's body could otherwise rewrite `author`/`source`/`source_ts` on an internal item) |
| An `external`-tier key CREATING an item that declares `team` | Clamped to `external`. The route derives tier from the payload, so without the clamp a client key could land content on every internal surface — retrieval context (which grounds LLM answers), dashboard `visibleItems`, team arcs/timeline. Clamped rather than refused: "an external key's content is external" is the right reading, and the `item.created` audit records the tier actually stored |
| Item redacted (body blanked) or re-pushed as a non-projectable `kind` | Same leak, different door: the projector reads the ledger BEFORE the "nothing to extract" skip and the default run scans every kind, so the group holding the pre-redaction episodes is recorded pending-delete and purged like any tier change. If the body is restored before the purge finishes, the projector purges-then-re-pushes and clears the flag, so the restored item is never silently deleted back out of the graph |
