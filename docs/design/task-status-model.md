...
|---|---|
| `ALTER TYPE … ADD VALUE` migration, since `pg:schema` cannot express it | `postgres/migrations/20260817120000_task_status_in_review.sql`, `add value if not exists … before 'blocked'`. Verified replaying twice through `npm run pg:schema` and from zero. |
...
| `activity-policy.ts` and its guard | `in_review` added to `ACTIVE_STATUSES`. This is not optional: those tasks were already counted as active (as `in_progress`), so omitting it would have removed every awaiting-review task from Home, Pulse in-flight and the timeline — a regression disguised as a fidelity gain. |
| a new board column for every team | Accepted. `components/kanban/types.ts` + `column.tsx`. |
| `pulse.ts` FUNNEL_ORDER, `scripts/brain-tasks.ts` | Both updated; `brain-tasks.ts` now *imports* `TASK_STATUSES` rather than keeping a fourth hand-maintained copy. |
...
| a `brain-api.md` version bump | 1.20 → **1.21**. See `lib/api/version.ts` for the full note, including the one observable break (a row an old client read as `in_progress` now reads as `in_review`). |
