...
`canSeeProjectRow(db, principal, projectId)` is the by-id form for the detail page (the
`canSeeItem` shape: shared predicate, no drift). Fail-closed: no member, read error → empty
set / false → `notFound()` / empty list. The dropdowns and the list page consume the SAME
resolution as their page's existing oracle call where one exists (the board already resolves
`visibleItemIds`; `visibleItemIds` already returns `projectIds` — reuse, don't recompute).
...
2. Same file — project DETAIL (D3): non-grantee gets `notFound()` for a restricted
   ...
   `/library/{itemId}` page (ENFB-1's gate) while the container 404s.
