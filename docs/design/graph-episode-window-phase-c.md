...
|---|---|---|
| **day 0 — sanity-read the windowed count** (the rule above, given a row here so it cannot fall between documents): for one group, compare the card's windowed entity count against the same group's **unfiltered** `MATCH (n:Entity {group_id: $g}) RETURN count(n)` | windowed ≤ unfiltered, and windowed **> 0** on a group with recent episodes | windowed **0** (or implausibly small) beside a healthy unfiltered count ⇒ nodes are dropping out of the window — a missing/unparseable `created_at`, or a `datetime()` comparison that is not matching what graphiti writes. That is the "parser that matches nothing reports zero" class, and every later reading in this table would inherit it. One read, free, and it either confirms the filter or exposes it. **Do this before recording any day-3 number.** |
