...
| `lib/projects/context/backfill.ts:168` count-skip | FIXED (program AC): the skip re-keys to counting DISTINCT covered `source_item_id`s vs `itemCount` — two aggregates, exact (multi-membership can no longer mask an uncovered item), and O(aggregates) per tick rather than the per-item probe's batch scans (cold-read M5 rejected the unmeasured probe-based claim) |
...
4. The count-skip fix is dm-pinned in the existing backfill suite: a team with `memCount >=
   itemCount` but one unpartitioned item is NOT skipped (the latent-bug fixture the program
