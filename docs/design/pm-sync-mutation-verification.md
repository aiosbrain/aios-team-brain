...
- `test/pm-sync-mutation-verification.test.ts` — the create path never returns a report with an empty `providerResourceId`. (The cache half — `boot.issuesById`/`issuesByExt`/`opts.resolved` unwritten on refusal — holds structurally because the throw precedes those writes at `linear.ts:335-336`, and is stated as a code property rather than a pinned one: review found the criterion promised a test that did not exist.)
