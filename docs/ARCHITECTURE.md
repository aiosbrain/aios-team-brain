...
| Identities (author→member) | `member_emails` (email/git aliases) + `member_identities` (provider user ids: slack/linear/… ) (+ `members.github_login`/`avatar_url`) | `lib/admin/*` + GitHub sync (`lib/codebases/github`) for emails; **`lib/identity/member-identities.setMemberIdentity` only** for provider ids (Slack auto-sync by email in `lib/ingest/sources/slack-identity` + manual `linkMemberSlack` on Admin → Members) | **`lib/identity/resolve`** (the one resolver: `byEmail` + `byProviderId`) → `lib/codebases/ingest`, `lib/ingest` Slack attribution, costs/maturity, dashboard | team-scoped one-to-one (`unique(team_id,email)` / `unique(team_id,provider,external_id)`); cross-member remap blocked unless forced. **Slack auto-map needs `users:read.email`**; else map manually |
...
`member_identities` · `integrations` · `agentic_maturity_snapshots` · `task_pm_links` · `work_events` ·
`usage_costs` · `graph_episodes`
<!-- /drift:tables -->
