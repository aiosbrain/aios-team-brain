...
| Meeting notes | `meeting_notes` + `meeting_note_attendees` (rich metadata layer; the full transcript itself is a normal `items` row, `kind='transcript'`, written through the existing `lib/ingest.ingestItem` single writer — so a meeting note is searchable/queryable through the standard FTS/retrieve pipeline for free) | **`lib/meetings/notes.ts` only** (single-writer guarded) — `createMeetingNote`, called from the Meetings upload form (`app/t/[team]/meetings/actions.ts`). Attendees + a short summary are inferred by a best-effort LLM pass (`lib/meetings/llm-extract`, mirrors `lib/graph/arcs.ts`'s provider pattern; never blocks the upload). Explicit `- [ ] ...` todos in the transcript are then picked up by the existing `lib/meetings/extract-todos` scan, scoped to just that note's item | Meetings list/detail pages (`app/t/[team]/meetings`, `app/t/[team]/meetings/[id]`) | team-tier only by design — `canSeeMeetingNotes` gate returns `[]`/`null` for `external` (sole enforcement, no RLS backstop); guarded by `test/guards/single-writer-meeting-notes` + the meeting-notes data-mechanics test |
...
`conversations` · `chat_messages` · `ingest_runs` · `meeting_notes` · `meeting_note_attendees`
<!-- /drift:tables -->
