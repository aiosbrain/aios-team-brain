...
| `datamechanics-tests` | `npm run test:migrate-from-existing` + `npm run test:datamechanics` against real Postgres 16 — schema upgrade-from-a-released-tag, RLS, persistence, access control. Runs in a `node:20-bookworm` job container reaching the service DIRECTLY at `postgres:5432` (a host-published port is Docker NAT, which the staging destination check correctly refuses), and installs the PGDG Postgres 16 client the specs spawn. Because the container's implicit `run` shell is Debian's dash (no `pipefail`), the job sets `defaults.run.shell: bash` — the toolchain steps' `set -euo pipefail` would otherwise abort with exit 2 before installing anything. The specs that need the peer-exact destination can be reproduced locally the same way with `npm run test:datamechanics:network` (a container on the database's own network); ordinary local data-mechanics runs are unchanged | Yes           |
...
**⛔ It runs on `pull_request_target`, and every line of code it executes comes from the DEFAULT branch** (the base branch, before GitHub's 2025-12-08 change) — the same shape, and for the same reason, as `pr-task-link.yml` below. The trigger used to be `pull_request: types: [closed]`, and on `pull_request` GitHub takes the **workflow file itself** from the PR head. So the `merged == true` guard was contributor-editable, and the attack needed no merge and no approval: push a branch, edit this file to delete the guard and print the three secrets, open a PR, then **close it unmerged** — `closed` fires on any close. Same class as `pr-task-link.yml`'s defect, one step worse, because the thing that was supposed to limit it lived inside the rewritable file.

What makes the current shape safe, in the order the properties matter:

- **Default-branch code only.** `actions/checkout` takes no `ref:` — on `pull_request_target` the default ref is the DEFAULT branch (the base branch, pre-2025-12-08) — and nothing from the PR head is fetched, installed, built or executed. The PR contributes **data only**: title, body, branch ref, head SHA, merge-commit SHA, html URL, author and merger, each passed through the step's `env:` and read with `process.env`. No `github.event.*` expression appears in a `run:` body, because interpolation happens before the shell parses and a PR title is therefore a command-injection primitive next to a durable credential.
- **`branches: [main, staging]` is not decoration — but the reason changed on 2025-12-08.** This used to say that without a filter a collaborator pushes `evil`, opens `evil2 → evil`, and the trusted base-branch code **is their own tree**. That is no longer possible: GitHub now takes the workflow file and checkout commit from the **default branch regardless of the pull request's base** ([changelog](https://github.blog/changelog/2025-11-07-actions-pull_request_target-and-environment-branch-protections-changes/)), so a contributor-chosen base cannot supply the trusted code. What the filter still buys is INTEGRITY over which merges count: with `["**"]` a collaborator merges their own PR into their own unprotected branch and `merged == true` fires from trusted code, posting a work event that can close a task.
...
**That cost is retired, and the paragraph that stated it was built on the pre-2025-12-08 model.** It said the run's ref is the PR's target branch, so a `staging` merge would be refused a `main`-only environment. On `pull_request_target` the environment is evaluated against the **default branch**, so the pull request's base never decides it. The policy is now `[main, staging]` and the default is `staging`. The hazard that replaced it: at a DEFAULT-BRANCH move, every `pull_request_target` run in the repo evaluates against the new default at once, so the policy must contain it first. `docs/RELEASING.md` §3.1c carries the entry.
...
- **`pull_request_target` runs the DEFAULT branch's workflow file and the DEFAULT branch's code** (it said *base* branch, and it checked out `main`; since GitHub's 2025-12-08 change the ref is the default branch, which is `staging` after the cutover). `actions/checkout` takes no `ref:`. The PR contributes **data only** — title, body, branch name, number — read out of `GITHUB_EVENT_PATH` as JSON and fed to a regex. It is never interpolated into a `run:` block (interpolation happens before the shell parses, which is a command-injection primitive next to a durable credential), and never selects what to check out or fetch.
- **`environment: trusted-automation` is a blast-radius control, NOT a trust boundary — and the distinction is the whole point.** It limits **which workflows in this repo** can reach the credential. It does **not** keep the credential away from untrusted **authors**. A `pull_request_target` run has `GITHUB_REF` = the DEFAULT branch, satisfies the deployment branch policy because the policy contains it, and receives the environment's secrets — and **a fork PR receives them too**, because the ref is the default branch for every author, so all of them satisfy the policy regardless of who they are. (This was measured in a disposable repository as `refs/heads/main` against a `main`-only policy; the conclusion is unchanged, the mechanism moved on 2025-12-08 and the policy is now `[main, staging]`.) Anyone reading the environment as fork protection would be repeating this workflow's original mistake in a new place.

  So the only thing protecting this key is the rule above: **default-branch code only, no PR-head checkout/fetch/import/execute, no attacker-controlled value interpolated into a `run:` body.** Treat it as load-bearing, because it is the entire defence.

  The environment is also **not connected yet**: `AIOS_API_KEY`, `AIOS_BRAIN_URL` and `AIOS_TEAM` still exist as plain repository secrets, and GitHub gives those to every job whether or not it names an environment — an environment secret only *shadows* a same-named repository secret, it does not revoke access to one. Connecting it means entering the values there **and deleting the repository-level copies**. That deletion used to be blocked on `aios-work-sync.yml`, whose `pull_request: closed` runs had ref `refs/pull/N/merge` that a `main`-only policy would refuse; it no longer is. **All three `AIOS_*` consumers — this file, `aios-work-sync.yml` and `scan-on-merge.yml` — now name the environment**, which `test/guards/pr-task-link-credential-isolation.test.ts` asserts by enumerating every job with one of the three secrets in scope rather than by naming the three files. So the deletion is unblocked and is a **repository-admin action**, not a code change. The trigger here is now `branches: [main, staging]` and the policy is `[main, staging]`. Both statements in the sentence this replaced were false after the cutover: the trigger was not pinned to `[main]`, and the trigger list and the branch policy do **not** move together on `pull_request_target` — the environment follows the **default branch**, so the policy must contain the default before it moves. `docs/RELEASING.md` §3.1c carries the corrected entry.
...
**Required secrets — the SAME three as `aios-work-sync.yml`:** `AIOS_BRAIN_URL`, `AIOS_TEAM`, `AIOS_API_KEY`. The job names `environment: trusted-automation` for the same reason the other two do — enrolment, so the repository-level copies can be deleted; it is not a boundary of any kind here, since this workflow is not PR-reachable at all. Its trigger is `push`, so the run's ref is the pushed branch — and unlike its two `pull_request_target` siblings, the 2025-12-08 default-branch change does **not** apply to it. MEASURED 2026-09-05, while the policy still allowed `main` only: two pushes to `staging` were refused the environment and the job went red in 3 seconds having run zero steps, with no scan posted. The policy is now `[main, staging]`; a future branch on this trigger needs the policy widened too.
...
### `staging-ops-image.yml` — manual ops runner image publisher (AIO-997)

**Not a gate and not part of any PR's checks.** It publishes `docker/staging-ops.Dockerfile` to the fixed private package `ghcr.io/aiosbrain/aios-staging-ops` and emits a digest receipt. Operator usage, the commissioning contract and the remaining account prerequisites live in `docs/OPS.md` §12; this entry records what it is in the CI inventory.

- **Trigger:** `workflow_dispatch` **only**, and it declares **no inputs** — there is no ref/sha/tag/destination for an operator to supply. The one legal source is the immutable `github.sha` of a manual dispatch on `refs/heads/staging` in `aiosbrain/aios-team-brain`. The first step is a **context-only shell guard that FAILS** (not a job-level `if`, which would render as a green skipped job) when the repository, event, ref, SHA shape, `github.workflow_sha` or `github.workflow_ref` is anything else. It runs before the checkout, so it never executes repository code to decide whether the repository context is trusted.
- **Permissions:** `contents: read` at the workflow level, `packages: write` added on the job — nothing else, ever. No `statuses`/`checks`/`contents` write (the context-forging scopes `test/guards/workflow-permissions.test.ts` exists for), no `id-token`, no `attestations`. It reads **no repository secret at all**: GHCR login uses the ephemeral `github.token`. `persist-credentials: false` on the checkout, and `.dockerignore` already excludes `.git`, so the Dockerfile's `COPY . .` cannot bake a token into a layer by either route.
- **No lifecycle reach.** It holds no Railway/AWS/Linear/Graphiti/database credential, moves no ref, touches no schedule or service variable, and spends nothing on a model. Publishing an artifact is **not** activation, deployment or promotion.
- **Identity is the registry digest.** `steps.build.outputs.digest`, never `outputs.imageid` (the local image *config* digest — same `sha256:<64-hex>` shape, names nothing in the registry). The helper then reads that exact by-digest reference back with `docker buildx imagetools inspect --raw` and **recomputes the digest from the returned bytes**, so a config digest cannot reach a success receipt. A top-level index/manifest-list descriptor refuses rather than guessing which child is the artifact — which is why `provenance: false` and `sbom: false` are set explicitly.
- **Honest accounting.** A GHCR metadata `404` means *absent OR invisible to this token* and is recorded as that ambiguity, never as "new package proven". After the push, only a measured private + `aiosbrain/aios-team-brain`-linked package supports a success receipt; anything else is **`published-unverified`** — the workflow fails, and nothing is deleted or made public to make the check pass. A push followed by a later failure is recorded as **partially published**; a retry publishes a new run-attempt tag and rolls nothing back.
- **Evidence delivery is accounted for separately, after the upload.** The failure record is written *before* the artifact upload, so an upload that fails after a verified publication — or a receipt written and then failed on its way to the job summary — would otherwise leave a red job whose only record says `published`. The last step reports a **`partially-published`** workflow outcome distinguishing `registry publication: confirmed|unverified` from `evidence delivery: incomplete`. It writes nothing: the measured receipt is not rewritten or demoted, nothing is deleted or re-pushed, and the run stays red for its original reason. The job log is the required surface; the summary append is best effort because it may be the failed transport. A **cancelled** run is unverified rather than clean — see `docs/OPS.md` §12.
- **Guarded by** `test/guards/staging-ops-image-workflow.test.ts` (the parsed workflow: trigger, refusal shape and step order, permissions, build context/platform, digest wiring, the failure-record gate, the build action's record-upload/summary opt-outs, tag uniqueness, receipt retention, the post-upload position and step-outcome inputs of the delivery report, fixed action allowlist) and `test/staging-ops-image-publication.test.ts` (the helper: every refusal above, plus the record-writing and reporting commands run end to end with the registry inspection, metadata transport, receipt file/summary, job log and clock injected — so the emit-before-throw ordering on the `published-unverified` path, and the partial account after an injected upload/summary failure, are exercised rather than assumed). Both are unit tier — no DB, no network, no registry.
  - **What the fixtures are.** Manifest cases are **correctly hashed fixtures**: small JSON documents whose sha256 is computed over their own bytes, which is exactly the property `confirmRegistryManifest` checks. They are not BuildKit-emitted manifests, and they prove parsing/hashing/descriptor behaviour only. That the real registry serves a shape this accepts stays a **postmerge live-run** question.

### `staging-ops-image-audit.yml` — manual read-only audit of ONE existing image (AIO-997)

**Not a gate and not part of any PR's checks.** It audits the single pinned digest of `ghcr.io/aiosbrain/aios-staging-ops` and emits one redacted evidence artifact, so a coordinator can decide whether the package may be made public. Operator usage, how to read the evidence and the transition order live in `docs/OPS.md` §13; this entry records what it is in the CI inventory.

- **Trigger:** `workflow_dispatch` **only**, with **no inputs** — no registry, repository, ref or digest for an operator to supply. The subject is a tuple in reviewed source (`scripts/staging-ops/image-audit/subject.mjs`), so "which artifact was audited" is answered by the commit. The first step is a **context-only shell guard that FAILS** on anything but a `workflow_dispatch` at a full 40-hex `github.sha` on `refs/heads/staging` in `aiosbrain/aios-team-brain`, with `workflow_sha == sha`. It expects **its own** `workflow_ref`, not the publisher's: reusing that path would accept a run of the publisher as though it were the audit. `set +x` explicitly, because a traced shell echoes context into a public log.
- **Permissions:** `contents: read` at the workflow level, **`packages: read`** on the job — never `packages: write`. It reads no repository secret; GHCR login uses the ephemeral `github.token`. There is no `docker/build-push-action` in the file at all, and the helper contains no write request, so the workflow **cannot** push, delete, re-tag or change a package's visibility. The operator does that by hand, afterwards, only once the evidence is accepted.
- **It never runs the image.** The export is read as bytes: no `docker run`, no entrypoint, no npm script. Static `User`/`Entrypoint` metadata and binary presence are **content inspection only** — they cannot close the Node/PG/`tini`, permissions or child-reaping rows, which stay separate measured commissioning evidence.
- **Identity is proved, not parsed.** The registry manifest is recomputed from its own bytes against the pinned digest; the config is hashed against that manifest's descriptor; each ordered layer is located **by digest** in the export (the export's own index is never read, so the walk is independent of the Docker version and image store) and verified to its `rootfs.diff_ids` entry, decoding **per declared media type** — a media type it cannot decode is refused by name rather than fed to gzip. A layer the export does not contain is a **missing layer** and fails.
- **Coverage is a return value.** Every layer's regular files are inventoried and staged for the scan, including files a later layer deletes or overwrites, plus the image config/history (where an `ENV` secret or a credential echoed into a `RUN` line lives). Nested `tar`/`gzip` members are expanded one level; anything deeper or in an unexpandable format is recorded as a **limitation**, which makes coverage incomplete and blocks the transition. Staging is bounded per layer *and* by ONE cumulative byte allowance across all layers and nested expansions — 64 layers each inside a per-layer limit is not a total — and exhausting it records a limitation instead of filling the runner's disk. Nothing is silently skipped.
- **The scan is pinned, and its report is validated.** gitleaks **8.28.0** by version *and* by the measured sha256 of its linux asset, with the binary's own `version`/`detect --help` measured on the runner before it is used. A report that is missing, unparseable, or JSON that is not a findings array is a **refusal** — never zero findings, which would be the best verdict the audit can produce handed to a broken run. A valid empty array is the genuine clean case. Findings' locations are normalized against the scan root (the pinned scanner reports **absolute** paths, measured) and a location that cannot be shown to sit inside it is retained as a finding with a fixed outcome and no raw path.
- **Redaction is an allowlist, not a filter.** Raw archives, extracted files, full config/history and every scanner match stay in ephemeral runner scratch — asserted to be **outside `GITHUB_WORKSPACE`**. Subprocess stdout/stderr is captured to scratch and never reaches the job log. The artifact carries digests, coverage counts, findings grouped by rule with **random per-run occurrence ids**, and a derived verdict; `buildEvidence` refuses to write a record that a leak guard finds anything sensitive in. A path is emitted only where independently established as public and non-sensitive.
- **`transitionReady` is derived and cannot be handed in.** A finding *or* an unmeasured thing sets it false, so "found nothing" and "did not look" do not read the same. An `unverified` package inventory (403/404/truncated pagination) blocks rather than counting as an empty package, and any additional version in the package blocks and is listed for a bounded audit. The measured **build recipe** is one of its inputs: a violated or unverified assertion blocks and lands on `unresolved` for adjudication, and a *missing* recipe blocks too, so a wiring slip can only cost a false block.
- **The publisher is unchanged (PUB-06).** Its private-only precheck, build/push path, receipt logic and tests are untouched; the only edit is an **optional, defaulted** `workflowPath` on the shared `dispatchContextFailures`. After an audited transition the publisher will refuse to publish again — deliberately, documented in OPS §13.
- **Guarded by** `test/guards/staging-ops-image-audit-workflow.test.ts` (the parsed workflow: trigger and no-inputs, the refusal's own workflow path, step order, the `packages: read` grant, absent build action, job timeout vs the helper's internal deadline, shared concurrency group, single always-uploaded artifact and its position; plus the dispatcher's single readiness call site and its recipe input) plus `staging-ops-image-audit-{archive,archive-surface,tar-structure,identity,content,scan-surface,evidence,reconcile,inventory,command,assembled-run}.test.ts` and `test/guards/image-audit-config-digest.test.ts` (the tracked scanner config against its pinned digest). All unit tier — no DB, no network, no registry, no Docker. `…-command.test.ts` spawns **real** processes for the scanner seam (a generated Node script that receives the audit's actual argument list) because a missing/invalid report, a deadline and an exhausted log buffer are properties of the boundary itself; it is a stand-in for one command, not a simulation of CI.
  - **What the fixtures are.** Hand-built tar archives and correctly hashed synthetic manifests/configs, with every secret-shaped value **minted per run**. `…-content.test.ts` drives a synthetic export through the real inspector end to end and demonstrates old-layer detection and a clean pass. **A mock pass is not the live audit**: none of it says anything about the real pinned digest, which is a **postmerge live-run** question.
