...
- **The privileged health token is bound BEFORE it is sent.** The deployment is read first; its `staticUrl` is the origin the probe is aimed at. No measured domain, or a configured `STAGING_ORIGIN` naming a different host, means **no health request is made at all** — a comparison performed after the fact cannot un-send a credential. The answer must also come back from that origin and report the observed deployment's commit, and the newest deployment edge must actually be `SUCCESS`: a building or failed deployment is not a serving identity.

**Its known gaps, stated rather than hidden.** Two kinds, and they are fixed by different people: an **operator prerequisite** (a read token, a comparison key, a pinned service ID that was not supplied — the notes say "prerequisite missing") and a **capability limit of this build** (there is no implemented read that could answer). The capability limits are: `graphiti-no-provider-credentials`, which needs an environment-scoped VARIABLE read this verifier deliberately does not carry; the app's outbound-connector configuration; live schedule state; branch/reference configuration; and the PROVENANCE of the opposite environment's fingerprint document, which arrives as plain JSON over no authenticated channel — a passing `credential-separation` therefore says "differ, as recorded in the supplied document". `app-no-model-spend` reads ONE reported field: it is evidence about answering posture, not about graph extraction, embedding, image or outbound policy. **This command cannot certify full activation, and does not claim to.** Confirm the unmeasurable controls during activation with their own probes and record the evidence; network isolation and object-store ACL rehearsal are likewise activation obligations — a JSON claim proves neither.
