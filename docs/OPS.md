...
**Activation is verified by a callable read-only check, not by a runbook checkbox.** `node scripts/staging-ops/importer.mjs activation-preflight` (`scripts/staging-ops/activation-preflight.mjs`) MEASURES the facts from the providers — each project token's own project/environment scope read back from Railway, both ops runners' pinned image digests and autodeploy state, the staging app's latest deployment bound to the pinned environment/service and its measured domain, the app's own privileged health answer (declared mode and answering posture), the credential-separation fingerprints against the opposite environment's signed document, and the schedules contract — then evaluates them. It needs no database, no locks and no runner role, because it is the check that runs BEFORE the thing it authorises exists.

Three properties are the point, and each replaces something weaker:

- **Every check reports a status, and an unmeasured one is `UNVERIFIED`.** The overall verdict is `ACTIVATED`, `NOT ACTIVATED` (we looked and it is wrong) or `UNVERIFIED` (we could not look) — and `assertActivated` refuses the last two alike. The previous `assertStagingTopology` could express neither: it validated a supplied JSON document, had no production caller, and a document says whatever it is written to say.
- **Operator claims are recorded and satisfy nothing.** `ACTIVATION_CLAIM_*` variables appear in the report labelled *not evidence*. Nothing an operator asserts moves a check to `pass`. (`STAGING_COPY_MODE_ACTIVATED` remains a claim that can only make validation STRICTER, never permit anything.)
- **It cannot write.** Only three named read-only GraphQL documents may be issued; `assertReadOnlyDocument` refuses a mutation or an unlisted query before the request is built.

**Its known gap, stated rather than hidden:** `graphiti-no-provider-credentials` reports `UNVERIFIED`. Confirming the sidecar holds no provider keys needs an environment-scoped VARIABLE read, which this verifier deliberately does not carry; confirm it during activation and record the evidence. Network isolation and object-store ACL rehearsal are likewise activation obligations with their own probes — a JSON claim proves neither.
