...
finite lifetime: every tick is rebudgeted independently.
...
**Activation is verified by a callable read-only check, not by a runbook checkbox.** `node scripts/staging-ops/importer.mjs activation-preflight` (`scripts/staging-ops/activation-preflight.mjs`) MEASURES the facts from the providers — each project token's own project/environment scope read back from Railway, both ops runners' pinned image digests and autodeploy state, the staging app's latest deployment bound to the pinned environment/service and its measured domain, the app's own privileged health answer at THAT domain (declared mode and answering posture), the credential-separation fingerprints against the opposite environment's supplied document, and the shipped schedules contract file — then evaluates them. It needs no database, no locks and no runner role, because it is the check that runs BEFORE the thing it authorises exists.
...
- **Operator claims are recorded and satisfy nothing.** `ACTIVATION_CLAIM_*` variables appear in the report labelled *not evidence*. Nothing an operator asserts moves a check to `pass`. (`STAGING_COPY_MODE_ACTIVATED` remains a claim that can only make validation STRICTER, never permit anything.) The **topology file is a document of expected pins**, and `STAGING_TOPOLOGY_MEASURED_FROM` is a label an operator typed, not provenance: `topology-identity` passes only when both project-token read-backs and the observed staging deployment corroborate the pinned project/environment/service, and it says explicitly that the branch/reference and internal-host pins are not read at all. `schedules-disabled-in-contract-file` reads the **shipped contract file** — it is a local configuration check and is never evidence about a live platform schedule.
- **It cannot write.** Only three named read-only GraphQL documents may be issued; `assertReadOnlyDocument` refuses a mutation or an unlisted query before the request is built. The Railway reads authenticate with the environment-scoped **`Project-Access-Token`** header, the same contract `RailwayMaintenance` uses.
...
**Its known gaps, stated rather than hidden.** Two kinds, and they are fixed by different people: an **operator prerequisite** (a read token, a comparison key, a pinned service ID that was not supplied — the notes say "prerequisite missing") and a **capability limit of this build** (there is no implemented read that could answer). The capability limits are: `graphiti-no-provider-credentials`, which needs an environment-scoped VARIABLE read this verifier deliberately does not carry; the app's outbound-connector configuration; live schedule state; branch/reference configuration; and the PROVENANCE of the opposite environment's fingerprint document, which arrives as plain JSON over no authenticated channel. Because of that last one, **`credential-separation` can report `pass` only for a document carrying authenticated, environment-bound provenance, which this build cannot produce** — so in practice it reports `UNVERIFIED`, and the comparison is an explicitly LOCAL diagnostic. The asymmetry is deliberate: "they are identical" is worth acting on from any source and still FAILS, while "they differ" is exactly what a forged or stale file would also say, so **live credential separation is unverified** until there is authenticated evidence for both sides. `app-no-model-spend` reads ONE reported field: it is evidence about answering posture, not about graph extraction, embedding, image or outbound policy. **This command cannot certify full activation, and does not claim to.** Confirm the unmeasurable controls during activation with their own probes and record the evidence; network isolation and object-store ACL rehearsal are likewise activation obligations — a JSON claim proves neither.
