...
`scripts/staging-ops/exporter.mjs` holds one read-only repeatable-read snapshot, produces the sanitized public dump/auth projection and fixed-read typed graph, signs the canonical manifest, and encrypts the payload. `scripts/staging-ops/importer.mjs` verifies and privately pins the immutable object before drain, requires the last-ready rollback pair, stops only read-back-identified pinned staging app/Graphiti deployments through the sole Railway adapter, holds the exclusive data lock, installs both stores, reapplies allowlisted tester passwords, verifies sanitation, boots the exact observed staging head, and advances ready only after an authenticated boot probe plus durable candidate readback. It repairs a stale durable pointer on a same-run retry, retains the prior pair through replacement, and deletes the prior/candidate rollback objects only after the new pair is ready. Catch-up is persisted when staging advances during maintenance. The Railway adapter reads the schema-supported auto-deploy status, enumerates every deployment page, rejects unknown statuses/ambiguous source commits, treats `REMOVING` as poll-only, and handles `NEEDS_APPROVAL` through the cancel path.
