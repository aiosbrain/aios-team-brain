...
describe("agentic-maturity snapshot ingest (real Postgres)", () => {
  ...
  // brain-api 1.11 (AIO-672): the workspace has been pushing this since 1.11 shipped; the
  // brain must persist it verbatim into BOTH the rollup score column and the jsonb summary.
  describe("context_health (1.11 scan summary)", () => {
    ...
    it("persists both the score column and the full jsonb summary", async () => {
      ...
      expect(row!.context_health).toEqual(contextHealth());
    });
    ...
    it("a later push replaces the stored summary in place", async () => {
      const seed = await seedTeam();
      await ingest(seed, snapshot({ context_health: contextHealth() }));
      await ingest(seed, snapshot({ context_health: contextHealth({ score: 1, drift_count: 5 }) }));
      const row = await rowFor(seed.teamId, seed.memberId, "2026-06-18");
      expect(row!.context_health_score).toBe(1);
      expect(row!.context_health).toMatchObject({ score: 1, drift_count: 5 });
    });
    ...
    // The 0-4 CHECK is a real DB constraint, not just a zod bound — prove it by writing
    // past the schema layer. (An out-of-range score can only arrive this way; the route
    // rejects it 422 first.)
    it("the DB rejects an out-of-range score even if the schema layer is bypassed", async () => {
      const seed = await seedTeam();
      await ingest(seed, snapshot());
      const { error } = await db()
        .from("agentic_maturity_snapshots")
        .update({ context_health_score: 9 })
        .eq("team_id", seed.teamId)
        .eq("member_id", seed.memberId)
        .eq("snapshot_date", "2026-06-18");
