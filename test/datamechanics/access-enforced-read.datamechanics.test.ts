...
// Phase B slice 1 (spec §5/§11), post-PRET-6: the ENFORCED read is the only read. The
// load-bearing proofs: a team member sees all backfilled General content, a member outside a
// restricting group cannot see an item tagged only into that restricted project, and placement
// is the sharing act (ruling 2). Fail-closed on no-groups.
...
// Two arms this suite used to open with are deleted WITH their subject (PRET-6):
//  - "permissive (default) is byte-identical … (the mode discriminator)" — the mode died.
//  - "a member removed from EVERY builtin … the permissive wall narrows to external rows" —
//    that narrowing was the PERMISSIVE posture wall. Under the one mode, no-groups means the
//    oracle serves nothing (pinned below); the surviving external-floor property (an
//    external-invited member sees exactly the external-shared corpus) is the leak suite's
//    invite-default-floor arm (membership-leak-suite, AC5).
describe("the enforced items read (one mode, post-PRET-6)", () => {
  it("a team member sees ALL backfilled General content", async () => {
  ...
  it("an outsider SEES General content but NOT a restricted-project item — pins the .in filter, not just the empty short-circuit (Fable HIGH)", async () => {
  ...
  it("a delegated agent token is membership-filtered too — cannot exceed its launcher (Fable HIGH H3)", async () => {
    ...
    // Assert the fixture, or the refusal asserted below passes through "group not found for team"
    // instead of through the guard — green while testing nothing (Codex diff review MEDIUM 1).
    expect(g2.ok, `fixture group: ${g2.error}`).toBe(true);
    expect(g2.groupId, "fixture group must have an id").toBeTruthy();
    const placed = await addMemberToGroup(db(), seed.teamId, g2.groupId!, agent, seed.memberId);
    expect(placed.ok, `fixture membership: ${placed.error}`).toBe(true);
    ...
    expect(got, "the agent must NOT see the restricted item its launcher can't (the project_id proxy is retired)").not.toContain("ag-secret.md");
    void shared;
  });

  it("a member in NO groups sees nothing (fail closed, not an unfiltered query)", async () => {
    const seed = await seedTeam();
    ...
    // mid-transition gap could): the read must serve zero rows, never fall back to unfiltered.
