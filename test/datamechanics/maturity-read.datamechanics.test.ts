...
describe("agentic-maturity dashboard reads (real Postgres)", () => {
  ...
  // brain-api 1.11 (AIO-672): the pushed context-health score must surface on the member
  // cards and roll up to a team average without disturbing the AEM placement rollups.
  ...
  it("exposes context_health_score on cards + averages it across the team (latest per member)", async () => {
    ...
    // alex: the earlier snapshot has no scan, the latest carries a score of 4 (latest wins).
    await ingest(seed.teamId, alex.id, snap("alex", "2026-06-16"));
  ...
  it("averageContextHealth is null until a member scans, and skips members with no scan", async () => {
    ...
    // Only alex scans → the average is alex alone. Unscanned sam is SKIPPED, not counted
    // as 0, which would otherwise report the team as unhealthy purely for not scanning.
  ...
  it("GATE: an external-tier viewer gets no context-health rollup either", async () => {
    ...
    const ext = await getTeamMaturity(db(), seed.teamId, "external");
    expect(ext.members).toEqual([]);
    expect(ext.averageContextHealth).toBeNull();
  });
});
