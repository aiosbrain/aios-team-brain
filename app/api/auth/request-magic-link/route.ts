 ...
 * regardless — with no provider configured, `sendMagicLink` uses the mailer's existing drop-and-log
 * behavior (lib/auth/mailer), same as every other email here.
 */
export async function POST(req: NextRequest) {
  ...
  after(async () => {
    try {
      ...
      console.error("[auth] magic-link delivery failed:", err instanceof Error ? err.message : err);
    }
  });
