...
> **Staging-first hardening status:** controller code is present but NOT ACTIVATED until the protected environments, two GitHub Apps, producer IDs and three verified main rulesets in `docs/RELEASING.md` are installed. After activation, steps 4–5 are performed only by manually dispatching `.github/workflows/release-controller.yml` from `staging` with the exact annotated tag, staging deployment ID, `copy-ready` mode and non-empty validation notes. Do not use the raw `git push` command in step 5. Before activation, stop and report the missing control; do not imply main enforcement exists.
