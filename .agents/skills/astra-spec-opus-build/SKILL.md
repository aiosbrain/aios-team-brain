...
description: Begin a requested build with a Linear ticket, attach the agreed spec to that ticket, then proceed through Astra specification, Claude Fable 5.1 spec and code reviews, Opus 5 implementation through the Claude CLI with Sol fallback on credit or usage limits where repository instructions and the user's builder assignment permit it, Astra adjudication and an independent final review, then push a branch and open a pull request. Use when the user requests this multi-model build workflow.
---
