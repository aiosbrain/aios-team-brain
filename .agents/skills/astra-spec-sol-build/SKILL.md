---
name: astra-spec-sol-build
description: Begin a requested build with a Linear ticket, attach the agreed spec to that ticket, then proceed through Astra specification, Claude Fable 5.1 spec and code reviews, GPT-5.6 Sol implementation through the Codex CLI with Opus fallback on credit or usage limits where repository instructions and the user's builder assignment permit it, Astra adjudication and an independent final review, then push a branch and open a pull request. Use when the user requests this multi-model build workflow.
---

# Astra-Spec-Sol-Build
...
## Repository and user policy precedence

This workflow is generic. The repository's `AGENTS.md`, this repository's other
instruction files, and the user's current builder assignment outrank every generic
instruction below whenever they conflict — including the builder-fallback and
handoff-storage instructions in this file. Before relying on any of them:

- **Builder assignment.** Use the builder the repository instructions and the user
  actually assign. Where `AGENTS.md` assigns Opus-only building and forbids automatic
  fallback, the fallback section below is NOT authorized: on a credit or usage limit,
  stop the active worker, checkpoint the work durably, and report the blocker instead
  of switching builders. Only an explicit local/user authorization re-enables a switch.
- **Subscription-only Anthropic authentication.** Never use an Anthropic API key,
  paid usage credits, or purchased resets for coding, including delegated coding and
  any authorized fallback. If subscription access is unavailable or limited, report
  the blocker.
- **Durable work and recovery.** The repository's durable-checkpoint rules govern
  storage. Never leave the only copy of implementation work, review findings, or
  verification evidence in `/tmp`, `$TMPDIR`, or any automatically cleaned directory;
  keep it in the durable ignored location the repository requires. Where this file
  says "temporary storage", read it as that durable location plus the repository's
  checkpoint cadence.
- **Usage monitoring and pauses.** The repository's authoritative-utilization,
  90% threshold, missing-telemetry and resume rules apply as written; nothing below
  relaxes them.
...
| Initial implementer, including accepted fixes | GPT-5.6 Sol (`gpt-5.6-sol`) through `codex` CLI | high |
...
2. Check `codex exec --help`, `claude --help`, authentication availability, and model availability using installed tooling. Resolve the exact provider identifier for **Fable 5.1** from authoritative configuration or provider documentation. The `fable` alias alone does not prove version 5.1. Verify availability of the exact implementation model `gpt-5.6-sol`; a generic model alias alone does not prove the required model and version. Verify the alternate builder before its first use as well: Sol is `gpt-5.6-sol` through Codex and Opus 5 is `claude-opus-5` through Claude. The builder fallback below is pre-authorized for credit or usage limits only, and only where repository instructions and the user's builder assignment permit an alternative builder; under an Opus-only assignment a limit is a pause-and-report condition. For other model unavailability or changes to any review role, report the affected stage and request a substitute before completing that stage.
3. Keep a compact handoff directory outside the committed source tree, in the durable ignored location the repository's durable-work rules require — never as the sole copy in automatically cleaned temporary storage. Record the request, base SHA, working directory, spec version, stage status, session identifiers, actual model identifiers, review findings, decisions, and verification results. Store the durable spec in the repository's usual spec location when appropriate. Keep temporary prompts and transcripts out of the PR.
...
### Builder fallback on credit or usage limits (only where locally authorized)

This section applies only where the repository instructions and the user's current builder assignment permit an alternative builder. Under an Opus-only, no-automatic-fallback assignment it does not apply: stop, checkpoint durably, and report the blocker.
...
For GPT-5.6 Sol implementation and accepted fixes, run the Codex CLI from the implementation worktree with high reasoning and write access under the existing permission controls:
...
For Opus 5 fallback implementation and accepted fixes, run from the same implementation worktree:
...
## Stage 3: Implementation (GPT-5.6 Sol first, automatic builder fallback)
...
Astra validates findings against code and product intent. Send accepted corrections to the active implementer, applying the fallback policy above if it hits a limit and that fallback is locally authorized. Preserve a decision record for rejected findings with concrete reasons. Verify fixes and run affected tests. Obtain another Fable pass when corrections are substantial or its blocking concerns remain unresolved.
