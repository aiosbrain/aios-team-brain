...
description: Begin a requested build with a Linear ticket, attach the agreed spec to that ticket, then proceed through Astra specification, Claude Fable 5.1 spec and code reviews, GPT-5.6 Sol implementation through the Codex CLI with automatic Opus fallback on credit or usage limits, Astra adjudication and an independent final review, then push a branch and open a pull request. Use when the user requests this multi-model build workflow.
---
